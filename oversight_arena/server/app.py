...
FIXES vs v4:
  ...
  FIX 6 (new) — /reset now returns BOTH `observation` AND `state` in the
    response body. Previously the return dict was truncated/empty, so the
    frontend could never read malicious_workers from the reset response and
    always fell back to EMPTY_STATE (all workers shown as honest).
...
def _state_dict() -> Dict[str, Any]:
    ...
    tr = getattr(state, "terminal_reward", None)
    data.setdefault("terminal_reward", float(tr) if tr is not None else None)
...
def reset(req: Optional[ResetRequest] = None) -> Dict[str, Any]:
    ...
    # FIX 6: Return BOTH observation AND state so the frontend can read
    # malicious_workers without a separate /state call.
    # Previously this return dict was empty/truncated — causing the frontend
    # to always fall back to EMPTY_STATE (all workers shown as honest).
    obs_dict = _dump(obs)
...
@app.post("/step")
def step(action: OverseerAction) -> Dict[str, Any]:
    # FIX 2: return 400 if called before /reset
    if not _env.state.episode_id:
        raise HTTPException(
            status_code=400,
            detail="No active episode. Call POST /reset first.",
