...
This script answers the question every judge will ask:
    "Can your RL-trained 1.5B model beat a paid frontier LLM at this task?"
...
Output:
    results/eval_gpt.csv
    results/eval_gpt_summary.json

Cost note: at 20 seeds × ~1500 input tokens × gpt-4o-mini pricing this run is
sub-dollar. Bump to gpt-4o for the real headline number once you've confirmed
the pipeline.
"""
...
Output rules:
  ...
  - Spurious flags carry a false-positive penalty. Acting too late carries an
...
# --------------------------------------------------------------------------- #
# Action parsing — same routine as scripts/train_grpo.py                       #
# --------------------------------------------------------------------------- #
...
def parse_action(text: str):
    ...
    return {
        ...
        "reasoning": raw[:240], "cwe_tag": "",
    }, False


# --------------------------------------------------------------------------- #
# Env helpers — same routine as scripts/train_grpo.py                          #
# --------------------------------------------------------------------------- #
...
def env_episode_reward(session, env_url, seed, difficulty, action) -> Dict[str, Any]:
    url = env_url.rstrip("/")
    try:
        r = session.post(f"{url}/reset", json={"seed": int(seed), "difficulty": float(difficulty)}, timeout=30)
        r.raise_for_status()
        ...
        return {"reward": -0.5, "f1": 0.0, "tp": 0, "fp": 0, "fn": 0, "error": str(e)}
...
def call_openai(model: str, diff: str) -> str:
    """Minimal OpenAI Chat Completions call. Avoids the SDK to keep deps light."""
...
def main():
    ...
    for i, seed in enumerate(seeds):
        try:
            ...
            text = call_openai(args.model, diff)
            ...
            print(f"[{i+1}/{len(seeds)}] seed={seed} action={action['action']:>15} -> reward={rows[-1]['reward']:.3f} f1={rows[-1]['f1']:.3f}")
            time.sleep(0.4)  # be polite to the API
        except Exception as e:
            print(f"[{i+1}/{len(seeds)}] seed={seed} FAILED: {e}")
            rows.append({
                "seed": seed, "difficulty": args.difficulty, "model": args.model,
                "valid_json": 0, "action": "", "worker_id": "", "reward": -0.5,
                "f1": 0.0, "tp": 0, "fp": 0, "fn": 0, "raw": str(e)[:400],
            })

    # CSV
    csv_path = RESULTS_DIR / "eval_gpt.csv"
    ...
    with open(RESULTS_DIR / "eval_gpt_summary.json", "w", encoding="utf-8") as f:
